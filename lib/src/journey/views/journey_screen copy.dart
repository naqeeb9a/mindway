...
class _JourneyScreenState extends State<JourneyScreen> {
  ...
  var moodVsFeelGood;
  var moodVsFeelBad;
  Map<String, int>? feelVsFactorMap;
  bool feelVsFactorData = false;
  ...
  getYearlyFactor(thisYear) async {
    ...
    if (snapshot.data() != null) {
      ...
      List<String> feelAndFactorArray = [];

      if (feelNames.isEmpty && factorNames.isEmpty) {
        return;
      }

      feelArray = feelNames.expand((element) => element).toList();
      factorArray = factorNames.expand((element) => element).toList();

      for (var feel in feelArray) {
        for (var factor in factorArray) {
          feelAndFactorArray.add("${feel}_$factor");
        }
      }

      // print("Element Feel" + feelArray.toString());
      // print("Element Factor" + factorArray.toString());

      debugPrint("Element FeelFactor$feelAndFactorArray");

      feelVsFactorMap = LinkedHashMap<String, int>();
      feelAndFactorArray.forEach((element) {
        feelVsFactorMap![element] = (feelVsFactorMap![element] ?? 0) + 1;
      });
      debugPrint("Element $feelVsFactorMap");
      setState(() {
        feelVsFactorData = true;
      });
    }
  ...
  buildFeelVsFactorData() {
    Map map = Map.from(feelVsFactorMap!);
    final sortedEntries = map.entries.toList()
      ..sort((e1, e2) => e2.value.compareTo(e1.value));
      ...
      for (int i = 0; i < sorted.length; i++)
        if (i < 5)
          Container(
            ...
            decoration: BoxDecoration(
              borderRadius: BorderRadius.circular(10),
              color: i.isEven ? Colors.white : Colors.grey.shade200,
            ),
            child: Padding(
              padding: const EdgeInsets.symmetric(horizontal: 0),
              child: Row(
                  ...
                  children: [
                    ...
                    Text(getPartName(sorted.keys.elementAt(i), 0)),
                    Text(
                      "+",
                      style: TextStyle(
                          fontWeight: FontWeight.bold, color: colorList[i]),
                    ),
                    Text(getPartName(sorted.keys.elementAt(i), 1)),
                    Text(sorted[sorted.keys.elementAt(i)].toString()),
                  ]),
            ),
          ),
